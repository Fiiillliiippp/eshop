...
const ProductCard = ({ product }: Props) => {
  ...
  const handleAddToCartClick = () => {
    ...
    const shippingPrice =  product.shipping === "Free" ? 0 : parseInt(product.shipping) 


    onAddToCartClick(product.id, product.price, product.title, shippingPrice)
  }
  ...
  return (
    ...
    >
      ...
      :
      null}
      <img alt={product.title} src={IMG(product.imageFront)} width='250px' height='250px' />
      <Typography variant='h5'>{product.title}</Typography>
      <Typography variant='caption'>{product.sizes}</Typography>
      <Typography variant='h6'>{product.price}€</Typography>
      <Button variant='contained' color='secondary' onClick={handleAddToCartClick}>
        Add to card
      </Button>
    </Box>
  );
